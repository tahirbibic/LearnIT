...
[
  {
    ...
    "correctIndex": 0 // index tačnog odgovora od 0 do 3
  }
]
...
${chatHistory || 'Nema transkripta, koristi samo tekst lekcije.'}`;
        ...
        let parsedQuestions = [];
        try {
           parsedQuestions = JSON.parse(jsonText) as Question[];
           if (!Array.isArray(parsedQuestions)) parsedQuestions = [];
        } catch (e) {
           console.error("Failed to parse", jsonText);
        }
        
        // Pobrini se da ima tačno 8 pitanja, ako AI vrati više ili manje
        const finalQuestions = parsedQuestions.slice(0, 8);
        while (finalQuestions.length < 8) {
           finalQuestions.push({
             q: "Koja je glavna tema ove lekcije iznad?",
        ...
        // Simuliraj da učenik rešava
        // Što je confusionScore veći, to su veće šanse da pogreši
        setTimeout(() => {
          const generatedAnswers: StudentAnswer[] = finalQuestions.map((q, idx) => {
             // Šansa za tačan odgovor
             // Ako je confusion 0 (savršeno), šansa je 95%
             // Ako je confusion 100 (užasno), šansa je 20%
             ...
             if (!isCorrect) {
                // Izaberi neki netačan nasumično
          ...
          setTimeout(() => {
            ...
            setGrade({
              score: correctCount,
              letter,
              iqEarned: correctCount * 10
            });
          }, 2000); // 2 sec smoke animation
        }, 3000); // 3 sec student taking exam
    ...
    generateQuiz();
  }, [lessonText, confusionScore, transcript, studentLevel]);
      ...
      <div className="relative w-full max-w-6xl aspect-video bg-[#3a2818] shadow-2xl overflow-hidden">
        ...
        <div id="exam-fallback" className="absolute inset-0 hidden flex-col items-center justify-center text-white text-center p-8 bg-[#8b5a2b]">
          ...
          <p className="font-pixel text-xl leading-relaxed max-w-2xl bg-black/40 p-6 rounded-lg text-left">
            Ubaci sliku papira za test u <strong>public/assets</strong> i nazovi je EXACTLY <strong>exam-bg.jpg</strong>.
          </p>
        </div>
             ...
             >
               <div className="relative flex items-center justify-center">
                 <motion.div 
                   ...
                   className="absolute w-64 h-64 bg-gray-300 rounded-full blur-2xl" 
                 />
                 <motion.div 
                   initial={{ scale: 0.2, opacity: 0.9, x: -20, y: -20 }} 
                   animate={{ scale: 3.5, opacity: 0, x: -150, y: -100 }} 
                   transition={{ duration: 1.3, ease: "easeOut" }}
                   className="absolute w-48 h-48 bg-white rounded-full blur-xl" 
                 />
                 <motion.div 
                   initial={{ scale: 0.2, opacity: 0.8, x: 20, y: 20 }} 
                   animate={{ scale: 3.2, opacity: 0, x: 150, y: 100 }} 
                   transition={{ duration: 1.4, ease: "easeOut" }}
                   className="absolute w-56 h-56 bg-gray-400 rounded-full blur-2xl" 
                 />
                 <motion.div 
                   initial={{ scale: 0.2, opacity: 0.9, x: 40, y: -10 }} 
                   animate={{ scale: 3.8, opacity: 0, x: 180, y: -80 }} 
                   transition={{ duration: 1.5, ease: "easeOut" }}
                   className="absolute w-52 h-52 bg-gray-200 rounded-full blur-xl" 
                 />
                 <motion.div 
                   initial={{ scale: 0.2, opacity: 0.8, x: -30, y: 40 }} 
                   animate={{ scale: 3.6, opacity: 0, x: -180, y: 120 }} 
                   transition={{ duration: 1.2, ease: "easeOut" }}
                   className="absolute w-60 h-60 bg-gray-300 rounded-full blur-3xl" 
                 />
               </div>
             </motion.div>
          )}
        ...
        {/* Papir za pitanja - Preko backgrounda */}
                  ...
                  return (
                    <div key={qIndex} className="relative h-1/4 w-full">
                       ...
                       <div className="absolute top-[2%] left-[15%] right-[2%] h-[31%] flex items-start overflow-hidden leading-[1.6]">
                          <span className="font-silkscreen text-[7px] md:text-[8px] lg:text-[10px] text-black/80">
                            {q.q}
                          </span>
                       </div>
                       ...
                       {q.options.map((opt, optIndex) => {
                         ...
                         // 6 lines total => option a is line 3 => 33.3%, b=50%, c=66.6%, d=83.3%
                         const topPos = [34, 50, 67, 83][optIndex];
                         
                         return (
                           <div key={optIndex} className="absolute left-[15%] right-[2%] h-[16%]" style={{ top: `${topPos}%` }}>
                              <span className={`absolute bottom-[2px] left-0 font-silkscreen text-[6.5px] md:text-[7.5px] lg:text-[9.5px] leading-none whitespace-nowrap overflow-hidden text-ellipsis max-w-full ${stage === 'results' && didStudentChooseThis ? 'font-bold text-black' : 'text-black/80'} ${stage === 'results' && isCorrectAnswer ? 'underline decoration-2' : ''} ${stage === 'results' && didStudentChooseThis && !ans?.isCorrect ? 'text-red-700' : ''} ${stage === 'results' && isCorrectAnswer ? 'text-green-700' : ''}`}>
