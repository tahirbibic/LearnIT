...
import { BookOpen, FolderOpen, ScrollText, History, FileText, Library, Upload, Loader } from 'lucide-react';
...
export function Classroom({ onBack, onGoToGreenboard, onHandOutExam, lessonText, setLessonText, iqPoints, studentLevel, history, activeStudent }: ClassroomProps) {
  ...
  const [folderTab, setFolderTab] = useState<'new' | 'biblioteka' | 'history'>('new');
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        ...
        const result = await generateContentProxy({
          ...
          contents: [{
            ...
            parts: [
              ...
              { text: "Izvuci sav tekst iz ovog dokumenta/slike koji bi bio koristan za lekciju. Vrati isključivo čitak tekst lekcije, bez dodatnih komentara ili formatiranja koje nije deo teksta." }
            ]
          }]
        });
      ...
      setTimeout(() => setShowSuccess(false), 3000);
       ...
       alert("Došlo je do greške pri čitanju fajla. Da li je fajl u podržanom formatu (PDF, JPG, PNG, TXT)?");
    }
    ...
    >
      <div className="relative w-full max-w-6xl aspect-video shadow-2xl overflow-hidden bg-[#76a5af]">
        ...
        <div id="classroom-fallback" className="absolute inset-0 hidden flex-col items-center justify-center text-white text-center p-8 bg-[#3a2818]">
          <p className="text-4xl mb-6 text-yellow-400 drop-shadow-md">Slika nedostaje!</p>
          <p className="font-pixel text-xl leading-relaxed max-w-2xl bg-black/40 p-6 rounded-lg text-left">
            Ubaci ovu novu sliku u <strong>public/assets</strong> i nazovi je EXACTLY <strong>classroom-bg.jpg</strong>.
          </p>
          
          {/* Fallback buttons in case image doesn't load */}
        ...
        {/* Interactive Invisible Buttons mapped to the image */}
        
        {/* Vrati se nazad */}
        <button 
           onClick={onBack}
           className="absolute left-[33.3%] top-[55.2%] w-[11.3%] h-[6.5%] bg-transparent hover:bg-white/20 cursor-pointer transition-colors border-2 border-transparent hover:border-green-400/80 rounded-md z-10"
        ...
        {/* Idi na tablu */}
        <button 
           onClick={onGoToGreenboard}
           className="absolute left-[46.5%] top-[55.2%] w-[11.3%] h-[6.5%] bg-transparent hover:bg-white/20 cursor-pointer transition-colors border-2 border-transparent hover:border-green-400/80 rounded-md z-10"
           title="Idi na tablu"
        >
        ...
        <button 
           ...
           className="absolute left-[37.4%] top-[36%] w-[12.6%] h-[18.2%] bg-transparent hover:bg-white/20 cursor-pointer transition-colors border-2 border-transparent hover:border-gray-300/80 rounded-sm z-10"
           title="Biblioteka i Materijali"
        >
        ...
        <button 
           ...
           className="absolute left-[38.2%] top-[21.5%] w-[11.8%] h-[14.5%] bg-transparent hover:bg-white/30 cursor-pointer transition-colors border-2 border-transparent hover:border-orange-400/80 rounded-sm z-10"
           title="Dnevnik studenta"
        >
        ...
        {/* Hand Out Exam */}
        <button 
           onClick={onHandOutExam}
           className="absolute left-[50.6%] top-[27.3%] w-[10.5%] h-[22.5%] bg-transparent hover:bg-white/20 cursor-pointer transition-colors border-2 border-transparent hover:border-red-400/80 rounded-sm z-10 rotate-[7deg]"
           title="Pokreni Test"
        >
          ...
          >
            <div className="bg-[#ebd09b] border-8 border-[#c2964e] w-full max-w-4xl min-h-[500px] p-0 text-[#5e411b] relative shadow-[12px_12px_0_rgba(0,0,0,0.5)] flex flex-col overflow-hidden">
              <button 
                ...
                className="absolute top-2 right-4 text-4xl font-bold hover:text-red-600 transition-colors z-[60]"
              >×</button>
                ...
                <div className="flex flex-col h-full font-pixel">
                  ...
                  <div className="flex bg-[#d4bb72] border-b-8 border-[#c2964e]">
                     <button 
                        ...
                        className={`px-8 py-4 font-silkscreen text-xl flex items-center gap-2 ${folderTab === 'new' ? 'bg-[#ebd09b] border-b-8 border-[#ebd09b] -mb-2' : 'hover:bg-[#e4ca8d]'}`}
                     >
                        <FileText size={20} /> NOVA LEKCIJA
                     </button>
                     <button 
                        onClick={() => setFolderTab('library')}
                        className={`px-8 py-4 font-silkscreen text-xl flex items-center gap-2 ${folderTab === 'library' ? 'bg-[#ebd09b] border-b-8 border-[#ebd09b] -mb-2' : 'hover:bg-[#e4ca8d]'}`}
                     >
                        <History size={20} /> BIBLIOTEKA ({history.length})
                     </button>
                  </div>
                    ...
                    {folderTab === 'new' ? (
                       <div className="flex flex-col h-full">
                          ...
                          <textarea 
                            ...
                            placeholder="Zalepljen tekst ili tvoje beleške... Marko će učiti iz ovoga!"
                          ></textarea>
                       ...
                       <div className="flex flex-col h-full">
                          ...
                          {history.length === 0 ? (
                             ...
                             <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar space-y-4">
                                {history.map((record) => (
                                   <div key={record.id} className="bg-[#f9f2e3] border-4 border-[#c2964e] p-6 flex justify-between items-center group hover:border-[#8b5a33] transition-all">
                     ...
                     <div className="flex-1 space-y-8">
                        ...
                        <div className="p-6 bg-[#d4bb72]/40 border-l-8 border-[#c2964e] relative overflow-hidden">
                           ...
                           <p className="text-lg leading-relaxed italic">
                              "Objasni kao da imam pet godina." – to je ključ. Ako koristiš previše komplikovanih reči, Marko će postati zbunjen i tvoj pedagoški skor će pasti. Koristi analogije iz svakodnevnog života!
                           </p>
                        </div>
                     </div>
                  </div>
