      ...
      <div className="relative w-full max-w-6xl aspect-video bg-[#3a2818] shadow-2xl overflow-hidden">
        ...
        <div id="greenboard-fallback" className="absolute inset-0 hidden flex-col items-center justify-center text-white text-center p-8 bg-[#3a2818]">
          ...
          <p className="font-pixel text-xl leading-relaxed max-w-2xl bg-black/40 p-6 rounded-lg text-left">
            Ubaci ovu novu sliku u <strong>public/assets</strong> i nazovi je EXACTLY <strong>greenboard-bg.jpg</strong>.
          </p>
          
          <div className="mt-8 flex gap-4">
             <button onClick={onBack} className="px-4 py-2 bg-red-600 text-white">Nazad</button>
             <button onClick={onTest} className="px-4 py-2 bg-green-600 text-white">Podeli testove</button>
             <button onClick={onTeach} className="px-4 py-2 bg-blue-600 text-white">Predaj lekciju</button>
          </div>
        </div>
        ...
        <button 
           ...
           title="Predaj lekciju"
        >
           <span className="sr-only">Predaj lekciju</span>
        </button>

        {/* Vrati se nazad (dugme koje nije na slici, ali mora da postoji) */}
        <button 
           onClick={onBack}
