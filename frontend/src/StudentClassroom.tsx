...
export function StudentClassroom({ onBack, onStartLearning, lessonText, setLessonText, learningLevel, setLearningLevel }: StudentClassroomProps) {
  ...
  const loadFromBiblioteka = async (doc: BibliotekaDoc) => {
    if (doc.extracted_text) {
      setLessonText(doc.extracted_text);
      setActivePopup(null);
      return;
    }
    setIsExtracting(true);
  ...
  return (
    <div className="w-full h-full relative font-silkscreen overflow-hidden bg-[#88a898]">
      ...
      {/* Preview box if content is loaded (overlaying the book) */}
      {lessonText && (
        <div className="absolute top-[28%] left-[37%] w-[26%] h-[24%] pointer-events-none opacity-40 overflow-hidden">
           <p className="text-[8px] font-pixel text-[#5d4037] text-center line-clamp-6 leading-tight">
             {lessonText}
           </p>
        </div>
      )}
      ...
      <div className="absolute top-10 right-10 flex gap-6 z-20">
        <button 
          onClick={onBack}
          className="px-6 py-4 bg-red-600 text-white border-4 border-red-800 hover:bg-red-500 shadow-[4px_4px_0_rgba(0,0,0,0.5)] flex items-center gap-2 transition-all hover:translate-y-1 hover:shadow-none"
        >
          <ArrowLeft size={20} />
          NAZAD
        </button>
        ...
        <button 
          ...
          disabled={!lessonText}
          className={`px-8 py-4 border-4 shadow-[4px_4px_0_rgba(0,0,0,0.5)] flex items-center gap-2 transition-all hover:translate-y-1 hover:shadow-none
            ${lessonText ? 'bg-green-600 border-green-800 text-white hover:bg-green-500' : 'bg-gray-400 border-gray-600 text-gray-300 opacity-50 cursor-not-allowed'}
          `}
        >
      ...
      <AnimatePresence>
        {activePopup === 'material' && (
          ...
          >
            ...
            >
              {/* Binder Rings Decor (Left Side) */}
              <div className="hidden md:flex flex-col justify-around py-8 px-2 bg-[#d7c4b1] border-r-4 border-[#5d4037] w-12">
                {[1, 2, 3, 4, 5, 6].map(i => (
                  <div key={i} className="w-6 h-6 rounded-full bg-gradient-to-br from-gray-300 to-gray-500 border-2 border-gray-600 shadow-inner" />
                ))}
              </div>
                 ...
                 <div className="flex justify-between items-center mb-6 border-b-4 border-[#5d4037] pb-4">
                    <div className="flex gap-2">
                      <button
                        ...
                        className={`px-6 py-3 font-silkscreen text-lg flex items-center gap-2 border-4 transition-all ${popupTab === 'upload' ? 'bg-[#5d4037] border-[#3d2b1f] text-white' : 'bg-white/40 border-[#5d4037]/30 text-[#5d4037] hover:bg-white/70'}`}
                      >
                        <BookOpen size={18} /> UPLOAD
                      </button>
                      <button
                        onClick={() => setPopupTab('biblioteka')}
                        className={`px-6 py-3 font-silkscreen text-lg flex items-center gap-2 border-4 transition-all ${popupTab === 'biblioteka' ? 'bg-[#5d4037] border-[#3d2b1f] text-white' : 'bg-white/40 border-[#5d4037]/30 text-[#5d4037] hover:bg-white/70'}`}
                      >
                        <Library size={18} /> BIBLIOTEKA
                      </button>
                    </div>
                    <button onClick={() => setActivePopup(null)} className="bg-[#5d4037] text-white p-2 hover:bg-red-700 transition-colors">
                      <X size={32} />
                    </button>
                 </div>
                 ...
                 <div className="flex flex-col lg:flex-row gap-8 flex-1">
                    <div className="flex-[2] flex flex-col gap-6">
                       <div className="flex gap-4">
                         <button
                           ...
                           className="flex-1 px-6 py-4 bg-[#3498db] text-white font-silkscreen hover:bg-[#2980b9] border-4 border-[#1a5276] transition-all shadow-[4px_4px_0_#1a5276] active:shadow-none active:translate-y-1 flex items-center justify-center gap-3"
                       ...
                       <div className="flex-1 flex flex-col min-h-[300px]">
                         ...
                         <textarea
                           ...
                           className="w-full flex-1 bg-[#fffcf5] border-4 border-[#5d4037] p-6 text-lg md:text-xl text-[#3d2b1f] resize-none focus:outline-none focus:border-[#d35400] transition-colors leading-relaxed custom-scrollbar shadow-inner"
                           placeholder="Ovdje nalepi tekst lekcije ili ga sam napiši..."
                         />
                       </div>
                    </div>
                    <div className="flex-1 flex flex-col gap-8">
                       <div className="bg-[#e8decb] border-4 border-[#5d4037] p-6 shadow-md">
                          ...
                          <div className="flex flex-col gap-3">
                            {(['basic', 'medium', 'advanced'] as const).map((level) => (
                              <button key={level} onClick={() => setLearningLevel(level)}
                                className={`w-full px-4 py-3 border-4 font-silkscreen text-sm transition-all text-left flex justify-between items-center ${learningLevel === level ? 'bg-[#5d4037] border-[#3d2b1f] text-white translate-x-2' : 'bg-white/50 border-[#5d4037]/30 text-[#5d4037]/60 hover:bg-white/80'}`}
                              >
                          ...
                          <div className="mt-4 p-3 bg-white/40 border-l-4 border-[#d35400] text-[10px] text-[#5d4037] leading-tight font-pixel">
                             {learningLevel === 'basic' && "Najjednostavniji jezik, idealno za kratak pregled."}
                             {learningLevel === 'medium' && "Balansirano predavanje sa objašnjenjima stručnih pojmova."}
                             {learningLevel === 'advanced' && "Duboka akademska analiza za napredne studente."}
                          </div>
                       </div>
                    ...
                    {isLoadingDocs ? (
                      ...
                      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-2">
                        {documents.map((doc) => (
                          <button
                            key={doc.id}
                            onClick={() => loadFromBiblioteka(doc)}
                            disabled={isExtracting}
                            className="w-full text-left bg-[#fffcf5] border-4 border-[#5d4037] p-5 hover:border-[#d35400] hover:bg-[#fff8ee] transition-all flex items-center gap-4 group disabled:opacity-50"
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs font-pixel text-[#8b5a33] mt-1">{doc.subject} · {Math.round(doc.file_size / 1024)} KB · {doc.extracted_text ? 'spremno' : 'učitava se pri prvom kliku'}</p>
                            </div>
